...
public class subListView extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Global.tempHeader.clear();
   		for(int i=0;i<Global.animalChild.get(Global.selectedAnimal).size();i++){
   		Global.tempHeader.add(Global.animalChild.get(Global.selectedAnimal).get(i).getName());
   		}
   	...
   	lvDisplayer.setOnItemClickListener(new AdapterView.OnItemClickListener() {
	        public void onItemClick(AdapterView<?> parent, final View view, int position, long id) {
	        	
        			Global.selectedAnimal= (String) parent.getItemAtPosition(position);
        			Global.choosenAnimal=Global.animalChild.get(Global.selectedHeadAnimal).get(position);
        		   Intent intent = new Intent("com.example.northamericanwildlifesounds.SOUNDDISPLAY");
